...
- [ ] `find-recording --work` is single-root: it calls `resolveWorkGroup` (first candidate only), so the same-title different-work trap (`"The Rite of Spring"` piano-transcription decoy, a real MB work) can still dead-end it; multi-root retry lives only in `ResolvePerformance`. Extend `resolveWorkGroups` retry to the find-recording path.
- [ ] Candidate-root walk climbs one `l_work_work` 281 level while group expansion is transitive: an alias hit on a movement of a multi-level family resolves a mid-level part as the root (live: St Matthew → "Matthäus-Passion, BWV 244: Teil II"), scoping `work.name`/recordings/first-year to the part. Curate is unaffected (materialization re-keys off the RG); fix = transitive ascent in `resolveWorkGroups` step (c).
- [ ] `workAliasCandidates` scan has no `ORDER BY` and a silent 50-id cap; add `ORDER BY work` + a truncation log if resolution flakiness is ever observed. Verify at the next re-curate: surname-first ("Bartók Béla") and native-script composer forms through the new alias-aware path.
