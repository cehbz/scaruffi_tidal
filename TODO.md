...
### Performance resolution — follow-ups (slice-2e MB spine shipped; Discogs query-time + domain model remain)
- Query-time Discogs discovery — **usable now** (the real-mirror full-scan hang is fixed). `discogsPerformances` candidate generation is index-driven, release-level: drive from the composer's `release_artist` credits (`idx_release_artist_artist`) + a release-level performer `EXISTS`; no more `FROM dc.master JOIN dc.release` scan, no `track_artist` arms (`track_artist.artist_id` is unindexed on the mirror). Full composer+conductor+orchestra query completes in ~1.2s warm (was >6 min). This is a MINIMUM fix. Remaining work:
  ...
  - [ ] **Real-data corroboration unverified.** The live Beethoven-5 / Bernstein / NYPhil query returns candidates/Medium with NO cross-source High — whether MB↔Discogs reconciliation actually fires on real anchors (year/title/bridge alignment) is open. The fix only made the query complete; it didn't touch matching. Validate + iterate (or supersede with the concordance).
  - [ ] **Latency (cold + prolific composer).** ~1.2s warm; worst-case ~25s cold for a maximally-prolific composer (Beethoven has ~100k `release_artist` credits, which the composer-driven query iterates). Optimization: drive candidate gen from the **least-frequent** constraint artist (a performer is usually far more selective) — touches the ≥1-vs-intersection semantics + confidence grading, so it wasn't the minimum fix.
