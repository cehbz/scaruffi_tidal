...
- [ ] Slice 5: render (rename realize→render) + diff/sync — the Python Tidal tool behind the JSON seam. **2026-07-02 second pass (staged pending commit approval): realized 173→204, gaps 71→40, errors 11→0** via the 256-char query clamp, GM album credits + credit-anchored `resolve_album` (per-credit anchors + any-credit survivor match), structured `gap_reason` on every gap, and bounded 5xx retry (a live transient window cost 25 entries before the retry). Remaining: the rename, diff/sync productization, and the 40 reasoned gaps (mostly track-fallback 0/N — genuine platform absence vs. per-track search quality, to re-examine after re-curate).
...
- **FTS candidate window — SHIPPED 2026-07-02** (staged pending commit approval): `resolveWorkGroup` candidate generation is composer-conditioned (`l_artist_work` 168 joined into the FTS query when the composer resolves), so generic titles no longer drown in same-titled floods and curate agents no longer need full canonical titles. Live: the Bernstein/NYPhil "Symphony no. 5" extreme resolves the full federated path in 9.1s (16 performances, 2 cross-source High; was `absent` at 80ms) — the 5,216-candidate worst case is measured and inside the interactive budget.
